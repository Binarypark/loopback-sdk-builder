...
module.exports = function generate(ctx) {
  ...
  let schema = [
     ...
     */
    {
      ...
      output: '/services/custom/sdk-models.ts',
      params: {
        models: ctx.models,
        kebabalize
      }
  ...
  if (ctx.isNgrx === 'enabled' || ctx.isNgrx === 'orm') {
    schema = schema.concat([
      ...
      {
        template: './shared/reducers/base-through.ejs',
    ...
    if (
      ...
      schema.push(
         ...
         */
        {
          ...
          output: '/models/' + kebabalize(modelName) + '.ts',
          params: {
      ...
      if (
        ...
        ctx.models[modelName].dataSource == null //transient classes are added as is and are not RestClasses
      ) {
        schema.push({
          template: './shared/services/custom/service.ejs',
          output: '/services/custom/' + kebabalize(modelName) + '.ts',
          params: {
            isIo: ctx.isIo,
            model: ctx.models[modelName],
            modelName: modelName,
      ...
      if (ctx.isNgrx === 'enabled' || ctx.isNgrx === 'orm') {
        schema.push({
          ...
          output: '/actions/' + kebabalize(modelName) + '.ts',
          params: {
            isIo: ctx.isIo,
            model: ctx.models[modelName],
            modelName: modelName,
        ...
        schema.push({
          ...
          output: '/effects/' + kebabalize(modelName) + '.ts',
          params: {
            isIo: ctx.isIo,
            model: ctx.models[modelName],
            modelName: modelName,
        ...
        schema.push({
          ...
          output: '/reducers/' + kebabalize(modelName) + '.ts',
          params: {
            isIo: ctx.isIo,
            model: ctx.models[modelName],
            modelName: modelName,
        ...
        schema.push({
          ...
          output: '/guards/' + kebabalize(modelName) + '.ts',
          params: {
            modelName: modelName
          }
        });
      ...
      if (ctx.isNgrx === 'orm') {
        schema.push({
          ...
          output: '/orm/models/' + kebabalize(modelName) + '.ts',
          params: {
            isIo: ctx.isIo,
            model: ctx.models[modelName],
            modelName: modelName,
  ...
  if (ctx.isNgrx === 'orm') {
    Object.keys(throughModels).forEach(modelName => {
      schema.push(
        {
          template: './shared/models/through-model.ejs',
          output: '/models/' + kebabalize(modelName) + '.ts',
          params: {
      ...
      schema.push({
        template: './shared/actions/through-model.ejs',
        output: '/actions/' + kebabalize(modelName) + '.ts',
        params: {
          modelName: modelName[0].toUpperCase() + modelName.slice(1)
        }
      ...
      schema.push({
        template: './shared/reducers/through-model.ejs',
        output: '/reducers/' + kebabalize(modelName) + '.ts',
        params: {
          modelName: modelName[0].toUpperCase() + modelName.slice(1)
        }
   ...
   */
  function buildModelImports(model, models) {
    ...
    Object.keys(model.properties).forEach(property => {
      var type = buildPropertyType(model.properties[property], models);
      var hasGeoPointType = type.indexOf('GeoPoint') !== -1;
      if (hasGeoPointType) {
        output.push('  GeoPoint');
      }

      if(models != null && models[type] != null) {
   ...
   */
  function buildModelProperties(model, isInterface, models) {
    let output = [];
  ...
  function buildServiceImports(model, loadAccessToken, isIo) {
    ...
    let imports = [
      ...
      { module: modelName, from: `../../models/${importName}`},
    ];
    if (isIo === 'enabled') {
      imports.push({
    ...
    getModelRelations(model).forEach((relationName, i) => {
      ...
      if (
        ...
        let importThrough = kebabalize(model.sharedClass.ctor.relations[relationName].modelThrough.sharedClass.name);
        if (!loaded[through] && targetClass !== modelName) {
          loaded[through] = true;
          imports.push({ module: through, from: `../../models/${importThrough}` });
        }
      }
      ...
      if (!loaded[targetClass] && targetClass !== modelName) {
        loaded[targetClass] = true;
        imports.push({ module: targetClass, from: `../../models/${kebabalize(targetClass)}` });
      }
    });
    ...
    model.methods.forEach(function(method) {
      if (typeof method.returns === 'object' && method.returns && method.returns[0]) {
        ...
        let excludes = ['any', 'object', 'number', 'string', 'date'];
        if(
          typeof type ==='string' &&
          excludes.includes(type.toLowerCase()) === false
        ){
          if(imports.find(t=>t.module === type) === false){
            imports.push({module:type,from:`../../models/${kebabalize(type)}`})
          }
        }
      }
    })
  ...
  function buildModuleImports(models, isIo, driver) {
    let imports = [
      ...
      { module: 'SDKModels', from: './services/custom/sdk-models'},
    ...
    Object.entries(models).forEach(([modelName, model]) => {
      ...
      let fileName = kebabalize(modelName);
      // transient models are not pulled from REST and are untampered models which still have dataSource
      if(model.dataSource == null) {

        imports.push({ module: `${name}Api`, from: `./services/custom/${fileName}` });
      }
    });
  ...
  function buildBaseServiceImports(isIo) {
    let imports = [
      ...
      { module: 'LoopBackConfig', from: '../../lb.config'},
      { module: 'LoopBackFilter, AccessToken', from: '../../models/base-models'},
      { module: 'SDKModels', from: '../custom/sdk-models'},
      { module: 'Observable, Subject', from: 'rxjs' },
   ...
   * Define which properties should be passed as route params
   */
  function buildPropertyType(property, models) {
    if (!property || !property.type) {
      return 'any';
    }
    switch (typeof property.type) {
      case 'function':
        if(models != null && models[property.type.name]) {
      ...
      case 'object':
        if (Array.isArray(property.type)) {
          return `Array<${buildPropertyType(property.type[0], models)}>`;
        }
        return 'object';
      default:
        return 'any';
